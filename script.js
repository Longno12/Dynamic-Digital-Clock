async function updateClock() {
    ...
    dateElement.textContent = now.toISOString().split('T')[0]; // Display date in YYYY-MM-DD format
...
async function updateWeather() {
    ...
    const location = 'London'; // Default location
...
function checkAlarm() {
    ...
    if (currentTime === alarmTime) {
        ...
        alarmTime = ''; // Reset the alarm
    }
}

function updateWorldClocks() {
    const now = new Date();
    const newYorkOffset = -4; // Adjust for daylight saving time
    const tokyoOffset = 9;
